...
class PasswordScanWorker(QObject):
    ...
    def _get_decrypted_passwords(self):
        ...
        fernet = get_fernet()
        return [
            (site, fernet.decrypt(pwd.encode()).decode())
            for site, pwd in encrypted_rows
            if self._running and self._safe_decrypt(fernet, pwd)
        ]

    def _safe_decrypt(self, fernet, pwd):
        try:
            fernet.decrypt(pwd.encode()).decode()
            return True
        except Exception:
            return False
...
class WeeklyChecker(QWidget):
    ...
    def init_ui(self):
        ...
        self._populate_passwords(layout)
        ...
        try:
            ...
            for site, encrypted_pwd in rows:
                try:
                    ...
                    layout.addLayout(self._password_row(site, pwd))
                except Exception:
                    continue
    ...
    def _password_row(self, site, pwd):
        layout = QHBoxLayout()
    ...
    def toggle_scheduled_scan(self, checked):
        self.scheduled_btn.setText("Disable Scheduled Scan" if checked else "Enable Scheduled Scan")
    ...
    def start_timer(self):
        if not self.timer:
            ...
            self.timer.start(5000)  # Replace with weekly interval
